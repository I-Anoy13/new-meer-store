...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [isAudioUnlocked, setIsAudioUnlocked] = useState(() => localStorage.getItem('itx_v18_unlocked') === 'true');
  const [customSound, setCustomSound] = useState<string | null>(() => localStorage.getItem('itx_v18_tone'));
  ...
  const triggerAlert = (name: string, amount: number) => {
    ...
    console.log(`[ALERT] New Order from ${name} at ${time}`);
    ...
    const interval = setInterval(() => {
      ...
      if (++count > 12) { clearInterval(interval); document.title = originalTitle; }
    }, 500);
    ...
    if (isAudioUnlocked) {
      ...
      if (chimeRef.current) {
        ...
        chimeRef.current.play().catch(e => console.warn("Audio blocked by OS", e));
      }
    }

    // C. Push Notification (Background capable)
    if ('serviceWorker' in navigator && Notification.permission === 'granted') {
      navigator.serviceWorker.ready.then(reg => {
        reg.active?.postMessage({
          ...
          options: {
            body: `Customer: ${name} — Tap to manage.`,
            tag: 'itx-v18-push',
            renotify: true
          }
        });
  ...
  const connectRealtime = () => {
    ...
    // Use a unique channel name to prevent "blinking" caused by session clashes
    const channelName = `admin_stream_${Math.random().toString(36).substring(7)}`;
    const channel = supabase.channel(channelName, {
      config: {
        broadcast: { self: true },
        presence: { key: 'admin-v18' }
      }
    });
    ...
    channel
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, (payload) => {
        ...
        setOrders(order);
        // Instant Alert
        triggerAlert(order.customer_name || 'Guest', order.total_pkr || order.total || 0);
      })
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          socketRetryRef.current = window.setTimeout(connectRealtime, 5000);
        }
      });
  ...
  // 4. Persistence Reactivation
  useEffect(() => {
    const onVisibilityChange = () => {
      setPermStatus(Notification.permission);
      if (document.visibilityState === 'visible') {
        refreshData();
        if (realtimeStatus === 'error') connectRealtime();
  ...
  // 5. THE GUARDIAN UNLOCK (Instant Gesture)
  const unlockGuardian = async () => {
    if (isArming) return;
    setIsArming(true);

    try {
      // Step 1: Initialize AudioContext IMMEDIATELY (browsers demand this)
      ...
      // Step 3: Direct Chime Warm-up
      if (chimeRef.current) {
        chimeRef.current.volume = 0.01; // Tiny volume test
      ...
      localStorage.setItem('itx_v18_unlocked', 'true');
      
      if (permission === 'granted') {
        triggerAlert("SYSTEM ARMED", 0);
      } else {
        alert("Alerts Enabled: Push notifications were blocked, but sound alerts are ACTIVE.");
      }
    } catch (err) {
      console.error("Unlock Failed", err);
      alert("Browser blocked activation. Please tap again and 'Allow' all permissions.");
    } finally {
      setIsArming(false);
    }
  };
      ...
      <div className="min-h-screen bg-[#1a1c1d] flex items-center justify-center p-6">
        <div className="w-full max-w-sm bg-white p-12 rounded-[3rem] shadow-2xl text-center">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if (adminPasswordInput === systemPassword) login(UserRole.ADMIN); }} className="space-y-6">
            ...
            />
            <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg hover:bg-black transition">Enter Console</button>
          </form>
        </div>
      </div>
    );
        ...
        <div className="p-6 border-t border-gray-800 space-y-3">
           {!isAudioUnlocked ? (
             <button 
               ...
               disabled={isArming}
               className={`w-full bg-blue-600 text-white p-5 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 shadow-lg shadow-blue-600/30 active:scale-95 transition-all ${isArming ? 'opacity-50 pointer-events-none' : 'animate-pulse'}`}
             >
               <i className={isArming ? "fas fa-circle-notch fa-spin" : "fas fa-shield-check"}></i>
             ...
             <div className="space-y-3">
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">System Health</span>
                    <div className={`w-1.5 h-1.5 rounded-full ${realtimeStatus === 'online' ? 'bg-green-500 animate-ping' : 'bg-red-500'}`}></div>
                  </div>
                  <p className="text-[9px] font-black text-white uppercase tracking-tight">Cloud: {realtimeStatus.toUpperCase()}</p>
                </div>
                <button onClick={() => triggerAlert("TEST ALERT", 0)} className="w-full bg-white/5 text-gray-500 hover:text-white p-3 rounded-xl text-[8px] font-black uppercase tracking-widest border border-white/5 transition">Test Chime</button>
             </div>
           )}
        </div>
      </aside>
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 shrink-0 z-50 shadow-sm">
          ...
          {lastAlertTime && (
            <div className="bg-blue-50 text-blue-600 px-5 py-2 rounded-full border border-blue-100 flex items-center gap-2 animate-fadeIn shadow-sm">
               <span className="text-[10px] font-black uppercase tracking-widest italic">Order Event: {lastAlertTime}</span>
            </div>
          )}
        </header>

        <main className="flex-grow overflow-y-auto p-6 md:p-12 animate-fadeIn custom-scrollbar bg-[#f6f6f7]">
          {activeNav === 'Home' && (
            <div className="max-w-7xl mx-auto space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                  <h2 className="text-4xl font-black tracking-tighter uppercase text-black italic">Master Console</h2>
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-2">Live Shop Operations</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  { label: 'Cloud Revenue', val: `Rs. ${totalSales.toLocaleString()}`, color: 'text-blue-600', icon: 'fa-money-bill-trend-up' },
                  { label: 'Capture Count', val: filteredOrders.length.toString(), color: 'text-black', icon: 'fa-shopping-basket' },
                  { label: 'Stream Link', val: realtimeStatus.toUpperCase(), color: realtimeStatus === 'online' ? 'text-green-500' : 'text-red-500', icon: 'fa-signal' },
                  { label: 'Guardian Status', val: isAudioUnlocked ? 'ARMED' : 'IDLE', color: isAudioUnlocked ? 'text-blue-600' : 'text-gray-300', icon: 'fa-user-shield' },
                ].map((stat, i) => (
              ...
              <div className="bg-white rounded-[3rem] border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-10 border-b border-gray-50 flex items-center justify-between bg-gray-50/10">
                   ...
                   <div className="flex items-center space-x-3">
                      ...
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{realtimeStatus === 'online' ? 'Listening' : 'Syncing'}</span>
                   </div>
                </div>
                <div className="divide-y divide-gray-50">
                  {filteredOrders.slice(0, 20).map(o => (
                    <div key={o.id} onClick={() => setViewingOrder(o)} className="p-8 flex justify-between items-center hover:bg-gray-50 cursor-pointer transition animate-fadeIn group">
                      <div className="min-w-0">
                        <p className="text-base font-black text-gray-900 group-hover:text-blue-600 transition truncate uppercase">#{o.id} — {o.customer.name}</p>
                        <p className="text-[10px] text-gray-400 font-bold mt-2 uppercase tracking-widest flex items-center gap-2">
                           <i className="fas fa-location-dot"></i> {o.customer.city || 'N/A'} • {new Date(o.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </p>
                      </div>
                  ...
                  {filteredOrders.length === 0 && <div className="p-40 text-center text-gray-300 uppercase text-[12px] font-black italic tracking-widest">Awaiting First Cloud Order...</div>}
                </div>
              </div>
            </div>
          )}
          ...
          {activeNav === 'Settings' && (
            <div className="max-w-2xl mx-auto space-y-10 pb-20">
               <h2 className="text-4xl font-black tracking-tighter uppercase text-black italic">Cloud Settings</h2>
               <div className="bg-white p-12 rounded-[3.5rem] border border-gray-200 shadow-xl space-y-12">
                  <div>
                    <h3 className="text-[11px] font-black uppercase text-gray-400 mb-8 tracking-widest">Notification Engine</h3>
                    <div className="space-y-6">
                      <div className="flex items-center justify-between p-8 bg-gray-50 rounded-[2.5rem] border border-gray-200">
                        <div className="flex items-center gap-6">
                           ...
                           <div>
                              <p className="text-sm font-black uppercase tracking-tight">Active Chime</p>
                              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{customSound ? 'User Custom MP3' : 'Standard ITX Tone'}</p>
                           </div>
                        </div>
                        ...
                        <input type="file" accept="audio/*" onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            reader.onload = (event) => {
                              ...
                              localStorage.setItem('itx_v18_tone', base64);
                              alert("Order chime updated successfully.");
                            };
                            reader.readAsDataURL(file);
                          }
                        }} className="absolute inset-0 opacity-0 cursor-pointer" />
                        <i className="fas fa-cloud-arrow-up text-4xl text-gray-300 mb-6 group-hover:text-blue-600 transition"></i>
                        <p className="text-[11px] font-black uppercase text-gray-400 tracking-widest">Upload Custom Order Sound (MP3)</p>
                      </div>
                    </div>
                  </div>
                  <div className="pt-12 border-t border-gray-100">
                     <h3 className="text-[11px] font-black uppercase text-gray-400 mb-8 tracking-widest">Security Override</h3>
          ...
          {activeNav === 'Orders' && (
            <div className="max-w-7xl mx-auto space-y-10">
              <h2 className="text-4xl font-black tracking-tighter uppercase text-black italic">Order Manifest</h2>
              <div className="bg-white rounded-[3rem] border border-gray-200 shadow-sm overflow-hidden">
                <table className="w-full text-left text-sm min-w-[900px]">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      <th className="px-10 py-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Order ID</th>
                      <th className="px-10 py-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Client Name</th>
                      <th className="px-10 py-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Location</th>
                      <th className="px-10 py-6 text-right text-[10px] font-black uppercase text-gray-400 tracking-widest">Grand Total</th>
                    </tr>
                  </thead>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                <div className="space-y-10">
                  <div className="bg-gray-50 rounded-[2.5rem] p-10 border border-gray-100 shadow-inner">
                    <h3 className="text-[11px] font-black uppercase text-gray-400 mb-8 tracking-widest flex items-center gap-2">
                       <i className="fas fa-box-open text-blue-600"></i> Package Contents
                    </h3>
                      ...
                      {viewingOrder.items.map((item, i) => (
                        <div key={i} className="flex items-center space-x-8 pb-8 border-b border-gray-200 last:border-0 last:pb-0">
                          ...
                          <div className="flex-grow min-w-0">
                            ...
                            <p className="text-[11px] text-gray-400 font-bold mt-2 uppercase tracking-widest">Units: {item.quantity} • {item.variantName}</p>
                          </div>
                ...
                <div className="space-y-10">
                  <div className="bg-white rounded-[2.5rem] border border-gray-100 p-10 shadow-xl">
                    <h3 className="text-[11px] font-black uppercase text-gray-400 mb-8 tracking-widest flex items-center gap-2">
                       <i className="fas fa-truck-fast text-blue-600"></i> Dispatch Details
                    </h3>
                    <div className="space-y-8">
                      <div>
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2 italic">Recipient</p>
                        <p className="text-lg font-black text-blue-600 uppercase truncate leading-none">{viewingOrder.customer.name}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-8">
                        <div>
                          <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2 italic">Contact</p>
                      ...
                      <div>
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2 italic">Destination</p>
                    ...
                    <div className="pt-10 border-t border-gray-100 mt-10">
                      <p className="text-[10px] font-black text-gray-400 uppercase mb-5 tracking-widest italic">Workflow Management</p>
                      <select 
