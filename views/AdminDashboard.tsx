...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [isAudioUnlocked, setIsAudioUnlocked] = useState(() => localStorage.getItem('itx_v17_unlocked') === 'true');
  const [customSound, setCustomSound] = useState<string | null>(() => localStorage.getItem('itx_v17_tone'));
  ...
  // 2. High-Priority Alert Execution
  const triggerAlert = (name: string, amount: number) => {
    ...
    const flashInterval = setInterval(() => {
      document.title = (flashCount % 2 === 0) ? "🚨 NEW ORDER! 🚨" : "ITX STORE";
      flashCount++;
      if (flashCount > 12) {
    ...
    if (isAudioUnlocked) {
      ...
      if (chimeRef.current) {
        ...
        chimeRef.current.play().catch(() => {
          console.warn("Audio Context likely suspended. Tap any UI element to revive.");
        });
      }
    }
    ...
    if ('serviceWorker' in navigator && Notification.permission === 'granted') {
      navigator.serviceWorker.ready.then(reg => {
        reg.active?.postMessage({
          ...
          options: {
            body: `Customer: ${name} — Click to process now.`,
            tag: 'itx-v17-alert',
            renotify: true
          }
        });
  ...
  const initSocket = () => {
    ...
    const channel = supabase.channel('itx_v17_stream_final', {
      config: {
        broadcast: { self: true },
        presence: { key: `admin-${Math.random().toString(36).substring(7)}` }
      }
    });
    ...
    channel
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, (payload) => {
        const o = payload.new;
        setOrders(o);
        triggerAlert(o.customer_name || 'Guest', o.total_pkr || o.total || 0);
      })
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          reconnectTimeoutRef.current = window.setTimeout(initSocket, 5000);
        }
      });
  ...
  // 4. Focus Reactivation
  useEffect(() => {
    const handleReactivation = () => {
      setPermStatus(Notification.permission);
      if (document.visibilityState === 'visible') {
        refreshData();
        if (realtimeStatus === 'error') initSocket();
  ...
  // 5. Explicit System Unlock (Synchronous Entry Fix)
  const unlockSystem = async () => {
    if (isArming) return;
    setIsArming(true);

    try {
      // Step A: Immediate Audio Activation (SYNC)
      if (!audioContextRef.current) {
        audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)();
      }
      
      // Resume must be triggered in the same user-event stack
      await audioContextRef.current.resume();

      // Step B: Warm up Chime
      if (chimeRef.current) {
      ...
      // Step C: Request Notifications
      const permission = await Notification.requestPermission();
      setPermStatus(permission);

      // Successfully Unlocked
      setIsAudioUnlocked(true);
      localStorage.setItem('itx_v17_unlocked', 'true');
      
      if (permission === 'granted') {
        triggerAlert("GUARDIAN ARMED", 0);
      } else {
        alert("System Active: Notifications are blocked by your browser, but sound alerts will work.");
      }
    } catch (error) {
      console.error("[UNLOCK ERROR]", error);
      alert("Browser blocked activation. Please ensure ITX is allowed to play sound and try again.");
    } finally {
      setIsArming(false);
    }
  };
      ...
      <div className="min-h-screen bg-[#1a1c1d] flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm bg-white p-10 rounded-[2.5rem] shadow-2xl">
          <div className="text-center mb-10">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2">Secure Admin Access</p>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); if (adminPasswordInput === systemPassword) login(UserRole.ADMIN); }} className="space-y-6">
            ...
            />
            <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-black transition shadow-lg">Login to Console</button>
          </form>
        </div>
      </div>
    );
        ...
        <nav className="flex-grow px-3 py-6 space-y-1">
          {menuItems.map(item => (
            ...
            >
              ...
              {item.badge ? <span className="bg-white/10 px-2 py-0.5 rounded text-[9px] font-bold">{item.badge}</span> : null}
            </button>
          ))}
        </nav>
        ...
        <div className="p-4 border-t border-gray-800 space-y-2">
           {!isAudioUnlocked ? (
             <button 
               ...
               disabled={isArming}
               className={`w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 shadow-lg shadow-blue-600/30 transition-all ${isArming ? 'opacity-50' : 'animate-pulse hover:bg-blue-500'}`}
             >
               <i className={isArming ? "fas fa-circle-notch fa-spin" : "fas fa-shield-halved"}></i>
               {isArming ? "Unlocking..." : "Enable Sound Alerts"}
             </button>
           ) : (
             <div className="space-y-2">
                <div className="flex items-center space-x-3 p-3 bg-white/5 rounded-xl border border-white/10">
                  ...
                  <div className="text-left">
                    <p className="text-[9px] font-black text-white uppercase tracking-widest">Cloud Stream</p>
                    <p className="text-[8px] text-gray-400 font-bold uppercase">{realtimeStatus === 'online' ? 'Stable' : 'Connecting...'}</p>
                  </div>
                </div>
        ...
        <main className="flex-grow overflow-y-auto p-4 md:p-10 animate-fadeIn custom-scrollbar">
          {activeNav === 'Home' && (
            <div className="max-w-6xl mx-auto space-y-8">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h2 className="text-3xl font-black tracking-tighter uppercase text-black italic">Sales Guardian</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  { label: 'Security Mode', val: isAudioUnlocked ? 'ARMED' : 'IDLE', color: isAudioUnlocked ? 'text-blue-600' : 'text-gray-300' },
                ].map((stat, i) => (
              ...
              <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/10">
                   <h3 className="font-black uppercase text-[10px] tracking-widest text-black italic">Live Activity Feed</h3>
          ...
          {activeNav === 'Settings' && (
            <div className="max-w-2xl mx-auto space-y-8 pb-12">
               <h2 className="text-3xl font-black tracking-tighter uppercase text-black italic">Settings</h2>
                      ...
                      <div className="flex items-center justify-between p-6 bg-gray-50 rounded-[2rem] border border-gray-200">
                        <div className="flex items-center gap-5">
                           ...
                           <div>
                              ...
                              <p className="text-[9px] text-gray-400 font-bold uppercase">{customSound ? 'Custom Audio File' : 'System Default'}</p>
                           </div>
                        </div>
                        ...
                        <input type="file" accept="audio/*" onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            reader.onload = (event) => {
                              ...
                              localStorage.setItem('itx_v17_tone', base64);
                              alert("Sound alert updated successfully.");
                            };
          ...
          {activeNav === 'Orders' && (
            <div className="max-w-6xl mx-auto space-y-6">
              <h2 className="text-3xl font-black tracking-tighter uppercase text-black italic">Transaction Log</h2>
