  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-gray-100">
      <div className="container mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="text-lg md:text-xl font-bold text-black tracking-tight uppercase flex items-center">
        ...
        <div className="flex items-center space-x-3 md:space-x-6">
          {user ? (
            <div className="flex items-center space-x-3 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">
              <span className="text-xs font-bold uppercase tracking-wide text-blue-600">Console: {user.name}</span>
            ...
            <div className="flex items-center space-x-4">
              <Link to="/cart" className="relative p-2 text-black hover:text-blue-600 transition">
                ...
                {cartCount > 0 && (
                  <span className="absolute top-0 right-0 bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-lg border border-white">
                    {cartCount}
                  </span>
                )}
              </Link>
